...
The number of input and output nodes are fixed per our categories and features, however there was still the question of how many hidden layers we should use and how many nodes each should have Having many layers allowed us to use multiple loss functions and transition between the dimensions of our features gradually. However adding too many layers or nodes often led to the model over-fitting. We tuned our model and found it worked best with 3 hidden layers, each with progressively smaller amounts of nodes.
#### Number of Epochs
Epochs are instnaces in which a neural net is trained on the input data. Over multiple instances, the network is more able to recognize meaningful patterns in the data and adjust closer to an optimal solution While using multiple epochs led to overall better performance, we found that around 50 epochs or more, the model tended toward over-fitting.


## Logistic Regression
### Description
-Logistic regression is a type of regression analysis used to predict the outcome of a categorical dependent variable based on one or more predictor variables. This can be extended to accommodate multiple classes, in what's called Multinomial Linear Regression. In our case, we used the multinomial logistic regression model to predict the genre of a song based on the features we extracted from the audio files. The model outputs the relative probabilities of each genre, from which the highest probability is chosen. The model is trained by adjusting the weights of the features according to their adherence to a loss function, in our case the softmax for reasons explained below, to minimize the error between the predicted genre and the true genre of the song. The model is then validated on a separate set of data to ensure that it generalizes well to unseen data.
### Tuning
In creating the logistic regression there were a few parameters that we had to set and tune in order to make our model work. These included the type of Linear regression we would do, the Maximum iterations allowed by the model, and The method in which the penalty was calculated for the regularizer.
#### Multiclass settings
Since Logistic regression is traditionally a binary classifier, we had to configure it to account for our 10 classes of data. This meant that we used Multinomial regression, which allows for classification of 3 or more different sets.
#### Maximum Iterations
The maximum number of iterations must be set in order to allow the model to successfully converge to the most accurate possible solution. We expiremented and found that, as we added features, we had to steadily increase the maximum iterations. In our final model, 300 iterations are used to let the model fully converge.
#### Penalty Norm
The regularizer is used to prevent over-fitting, and is calcualted by taking a norm of L2 was most effective in minimizing overfitting without taking too much away from the overall model.
